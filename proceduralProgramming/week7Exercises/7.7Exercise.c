...
int main(void)
{
  ...
  printf("please enter the word:\t"); //prompts the user to enter the word
  char wordBeingCounted[20];  //creates a variable to store the word being searched for
  scanf("%s", &wordBeingCounted); //stores the input in the relevant variable
  for (int i = 0; i < strlen(wordBeingCounted); i++)  //Traverses the word to ensure that no characters are capitalized
  {
    if ((int)wordBeingCounted[i] < 90)  //if any characters are capitalized they are changed to their lower case equivalent
    {
      wordBeingCounted[i] += 32;
    }
  }

  int wordInScentance = 0;  //variable that is 0 until the word is found in the array of words in the inputted scentance
  for (int i = 0; i < uniqueWordCount; i++) //Traverses the array of words in the scentance
  {
    if (strcmp(wordsInInput[i].word, wordBeingCounted) == 0)  // if the word entered is the same as a word stored in the array
    {
      wordInScentance++;  //incremented so the program knows that the word has been found in the array
      printf("\"%s\" occurs %i times in the inputted scentance\n", wordBeingCounted, wordsInInput[i].occurances);
    }
  }
  if (wordInScentance == 0) //output if the wordBeingCounted was not in the original scentance input
    printf("\"%s\" is not in the inputted scentance.\n", wordBeingCounted);
