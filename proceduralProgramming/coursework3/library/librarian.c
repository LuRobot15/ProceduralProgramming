...
void listBorrowedBooks( Book *bookList, int numBooks ) {
  ...
  for (int i = 0; i < numBooks; i++)  //traverse every book in the list
  {
    if (bookList[i].available == 0) //if the books "available" field is false
    {
      printf("%s - %s\n", bookList[i].author, bookList[i].title); //prints the book's details in required format
    }
  }
...
void listBooks( Book *bookList, int numBooks ) {
  ...
  for (int i = 0; i < numBooks; i++)  //traverses every book in the list
  {
    printf("%s - %s\n", bookList[i].author, bookList[i].title); //prints the book's details in required format
  }
